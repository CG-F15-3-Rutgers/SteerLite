...
*/
    ...
    if (testcase == "plane_egress") {
        plane_egress=true;
		_SocialForcesParams.sf_wall_b = 0.09;
		_SocialForcesParams.sf_wall_a = 31;

    } else if (testcase == "plane_ingress") {
        plane_ingress=true;
		_SocialForcesParams.sf_wall_b = .6;
		_SocialForcesParams.sf_wall_a = 27;
		_SocialForcesParams.sf_agent_a = 5;
		_SocialForcesParams.sf_agent_b = 2;
    } else if (testcase == "crowd_crossing") {
       crowd_crossing=true;
	   _SocialForcesParams.sf_wall_b = 1.2;
		_SocialForcesParams.sf_wall_a = 30;
		//_SocialForcesParams.sf_agent_a = 7;
		//_SocialForcesParams.sf_agent_b = 2;
		//_SocialForcesParams.sf_sliding_friction_force = 80;
	   ...
	   _SocialForcesParams.sf_wall_b = 1;
	   _SocialForcesParams.sf_wall_a = 30;
       ...
       wall_squeeze=true;
	   _SocialForcesParams.sf_wall_b = 1;
	   _SocialForcesParams.sf_wall_a = 24;
    } else if (testcase == "hallway-two-way") {
       hallway_two_way=true;
    } else if (testcase == "maze") {
        maze=true;
		ob = 2;
    } else if (testcase == "custom-bus"){
		custom_bus = true;
		_SocialForcesParams.sf_wall_b = .8;
		_SocialForcesParams.sf_wall_a = 24;
		_SocialForcesParams.sf_sliding_friction_force = 10;
	}
	...
	if(plane_egress || plane_ingress || office_complex || maze || doorway_two_way ||custom_bus||bottleneck_squeeze){
		AStarplanning();
	}else{
		runLongTermPlanning();
	}
...
Util::Vector SocialForcesAgent::calcProximityForce(float dt)
{
	...
	for(std::set<SteerLib::SpatialDatabaseItemPtr>::iterator neighbour = _neighbors.begin(); neighbour!= _neighbors.end(); neighbour++){
		if((*neighbour)->isAgent()){
			...
			/*
			float dist = Util::distanceBetween(tmp_agent->position(), this->position());
			float radi = tmp_agent->radius() + this->radius();
			float proxForce = sf_agent_a * std::exp((radi - dist) / sf_agent_b);
        
			Util::Vector norm = normalize(this->position() - tmp_agent->position());
			away += norm * proxForce; */
			if (_radius== 1) {
				away = away * 2;
			}
			...
			tmp_obj = dynamic_cast<SteerLib::ObstacleInterface*>(*neighbour);
...
Util:Vector GoalForce = Util::Vector(0, 0, 0);
	if (_radius == 1) {
		GoalForce = 100000000000000000* mass*((_goalDirection * PERFERED_SPEED) - velocity()) / _dt;
	}
	else {
		GoalForce = mass*((_goalDirection * PERFERED_SPEED) - velocity()) / _dt;
	}
...
Util::Vector SocialForcesAgent::calcAgentRepulsionForce(float dt)
{
	...
	for (std::set<SteerLib::SpatialDatabaseItemPtr>::iterator neighbor = _neighbors.begin(); neighbor != _neighbors.end(); neighbor++) {
		...
		if (tmp_agent->computePenetration(this->position(), this->radius()) > 0.000001) {
			if (this->radius() > .99) {
				returnVector = 100000000* returnVector + (tmp_agent->computePenetration(this->position(), this->radius()) * _SocialForcesParams.sf_agent_body_force * normalize(position() - tmp_agent->position()));
			}
			else {
...
// #define _DEBUG_ 1
	...
	Util::Vector acceleration = (prefForce + repulsionForce + proximityForce)/mass;
...
bool SocialForcesAgent::AStarplanning() 
{
    _midTermPath.clear();
    std::vector<Util::Point> agentPath;
    Util::Point pos = position();
    AStarPlanner Astarpath; 
    if (!Astarpath.computePath(agentPath, pos, _goalQueue.front().targetLocation, gSpatialDatabase, false, false,ob)) {
