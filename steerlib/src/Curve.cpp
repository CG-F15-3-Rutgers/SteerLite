...
bool sortPoints(CurvePoint &p1, CurvePoint &p2) {
	if (p1.time<p2.time) {
		return true;
	}
	else {
...
{
	std::sort(controlPoints.begin(), controlPoints.end(), sortPoints);
	return;
...
Point Curve::useHermiteCurve(const unsigned int nextPoint, const float time)
{
	...
	x2 = (t3 - 2 * t2 + t)*curveP[nextPoint - 1].tangent.x*dt;
	x3 = (t3 - t2)*curveP[nextPoint].tangent.x*dt;

	x = x0 + x1 + x2 + x3;

	y0 = (2 * t3 - 3 * t2 + 1)*prevP.y;		//hermite math for y
	y1 = (-2 * t3 + 3 * t2)*nextP.y;
	y2 = (t3 - 2 * t2 + t)*curveP[nextPoint - 1].tangent.y*dt;
	y3 = (t3 - t2)*curveP[nextPoint].tangent.y*dt;

	y = y0 + y1 + y2 + y3;

	z0 = (2 * t3 - 3 * t2 + 1)*prevP.z;		//hermite math for x
	z1 = (-2 * t3 + 3 * t2)*nextP.z;
	z2 = (t3 - 2 * t2 + t)*curveP[nextPoint - 1].tangent.z*dt;
	z3 = (t3 - t2)*curveP[nextPoint].tangent.z*dt;
